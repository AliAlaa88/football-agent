...
export const toolsSchema = [
  ...
  },
  {
    ...
    function: {
      name: "search_players",
      description: "Search for football players on Transfermarkt by name to get their player ID.",
      parameters: {
        type: "object",
        properties: {
          player_name: { type: "string", description: "The name of the player to search for." }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get the profile and biographical information of a football player using their player ID.",
      parameters: {
        type: "object",
        properties: {
          player_id: { type: "string", description: "The Transfermarkt player ID." }
        },
        required: ["player_id"]
      }
    }
  },
  {
    ...
    function: {
      ...
      description: "Get the performance statistics of a football player using their player ID.",
      parameters: {
        type: "object",
        properties: {
          player_id: { type: "string", description: "The Transfermarkt player ID." }
...
export async function executeTool(toolName, args, baseUrl) {
  ...
  const finalBaseUrl = baseUrl || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : (process.env.BASE_URL || 'http://localhost:3000'));

  if (toolName === "search_players") {
    try {
      const res = await fetch(`${finalBaseUrl}/api/tfmkt/players/search/${encodeURIComponent(args.player_name)}`);
      if (!res.ok) throw new Error("Transfermarkt API failed");
      const data = await res.json();
      return JSON.stringify(data);
    } catch (e) {
      return `Failed to search players: ${e.message}`;
    }
  }

  if (toolName === "get_player_profile") {
    try {
      const res = await fetch(`${finalBaseUrl}/api/tfmkt/players/${encodeURIComponent(args.player_id)}/profile`);
      if (!res.ok) throw new Error("Transfermarkt API failed");
      const data = await res.json();
      return JSON.stringify(data);
    } catch (e) {
      return `Failed to get player profile: ${e.message}`;
    }
  }

  if (toolName === "get_player_stats") {
    try {
      const res = await fetch(`${finalBaseUrl}/api/tfmkt/players/${encodeURIComponent(args.player_id)}/stats`);
      if (!res.ok) throw new Error("Transfermarkt API failed");
      const data = await res.json();
      return JSON.stringify(data);
    } catch (e) {
      return `Failed to get player stats: ${e.message}`;
    }
  }
