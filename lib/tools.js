...
export async function executeTool(toolName, args) {
  ...
  if (toolName === "search_players") {
    try {
      const res = await fetch(`${baseUrl}/api/tfmkt/players/search/${encodeURIComponent(args.player_name)}`);
  ...
  if (toolName === "get_player_profile") {
    try {
      const res = await fetch(`${baseUrl}/api/tfmkt/players/${encodeURIComponent(args.player_id)}/profile`);
      if (!res.ok) throw new Error("Transfermarkt API failed");
      const data = await res.json();
      return JSON.stringify(data);
    } catch (e) {
  ...
  if (toolName === "get_player_stats") {
    try {
      const res = await fetch(`${baseUrl}/api/tfmkt/players/${encodeURIComponent(args.player_id)}/stats`);
      if (!res.ok) throw new Error("Transfermarkt API failed");
      const data = await res.json();
      return JSON.stringify(data);
    } catch (e) {
